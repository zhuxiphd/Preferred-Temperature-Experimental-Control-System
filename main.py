...
from restore_audio import restore_audio_files
...
AUDIO_FILES = {
    ...
    "camera_only": AUDIO_DIR / "请设置好红外摄像机.mp3",
...
def play_audio(file_path: Path):
    """在后台线程播放音频。当前程序主要面向 macOS，使用 afplay 播放。"""
...
def play_voice_alert(audio_key_or_path):
    """播放指定语音提醒。兼容旧版本地绝对路径写法。"""
    if audio_key_or_path in AUDIO_FILES:
        audio_path = AUDIO_FILES[audio_key_or_path]
    else:
        # 兼容旧版写法：取文件名并改为项目内 audio/ 相对路径。
        audio_path = AUDIO_DIR / Path(str(audio_key_or_path)).name

    play_audio(audio_path)

def start_experiment(start_offset=0):
    """开始实验并更新进度"""
    global start_time, alert_times
    start_time = datetime.now() - timedelta(seconds=start_offset)  # 调整开始时间，包含秒数
    alert_times = {0, 20, 29, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 76, 80, 90}
...
def update_progress():
    """更新实验进度和计时器显示"""
    now = datetime.now()
    elapsed_time = (now - start_time).total_seconds() / 60  # 转换为分钟
...
def update_time_axis(minute):
    """根据当前分钟数更新时间轴颜色"""
    for min in range(minute + 1):
        if min < 31:
            fill_color = "#E5FEE2"
        elif min < total_minutes - 10 + 1 and min > 30:
            if min < total_minutes - 10 + 1:
                fill_color = "#E4F9FE"
            else:
                fill_color = "#FFD5ED"
        else:
            fill_color = "#FFD5ED"
        if min < len(segments):  # 确保不会超出 segments 的范围
            time_axis_canvas.itemconfig(segments[min], fill=fill_color)

def draw_time_axis(canvas):
    global segments
    segments = []  # 存储所有段的列表
    start_x = 50
    segment_width = (1100 - 100) / (total_minutes + 1)  # 调整宽度以适应所有分钟段
    ...
    for i in range(total_minutes + 1):
        rect = canvas.create_rectangle(start_x + i * segment_width, 40, start_x + (i + 1) * segment_width, 60, fill="#D0D0D0", outline="")
        segments.append(rect)

    # 画出刻度、标签和阶段名称
    tick_positions = [0, 20, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80, 90] + list(range(80, total_minutes + 1, 5))
    adjust_positions = [20, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80, 90] + list(range(80, total_minutes + 1, 5))
    phase_labels = [(17, "预暴露"), ((total_minutes + 22) // 2, "正式实验"), (total_minutes - 10 + 4, "代谢率测试")]
...
def update_time():
    global root  # 将 root 声明为全局变量
    now = datetime.now()
    date_label.config(text=now.strftime('%Y-%m-%d'))
    time_label.config(text=now.strftime('%H:%M:%S'))
    root.after(1000, update_time)

def focus_next_widget1(event):
    if len(hour_entry.get()) == 2:
        minute_entry.focus()

def check_time_and_update_button1(event=None):
    """检查输入的时间并更新按钮文本"""
    try:
...
def add_five_minutes():
    ...
    total_minutes += 5  # 增加5分钟
    alert_times = {0, 20, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80} | set(range(80, total_minutes + 1, 5))
    draw_time_axis(time_axis_canvas)
    update_time_axis(int((datetime.now() - start_time).total_seconds() / 60))  # 转换为分钟

def setup_gui():
    global root, date_label, time_label, hour_entry, minute_entry, start_button, time_axis_canvas, total_minutes
    root = tk.Tk()
    root.title("实验控制")
    root.geometry("1500x1000")

    total_minutes = 90  # 设置实验总时间为90分钟
    ...
    hour_entry.bind("<KeyRelease>", focus_next_widget1)
